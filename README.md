...
📝 說明：慢性病以糖尿病居多；APOE_ε4 基因陽性過半；目標變數失智症（Dementia 0/1）比例約 1:1，資料分佈無失衡。
...
📝 說明：由於特徵多為二元與等級變數，非 PCA 最佳適用的連續型變數，導致累積解釋變異量僅達 42.93%。
...
📝 說明：雖說原始資料分佈已經相當平均，但還是利用過採樣將訓練集平衡，使兩者的學習機會相等。
...
📝 說明：評估結果顯示**羅吉斯迴歸**在準確率（CV Acc = *0.745 ± 0.022*）與 **F1 分數**（CV F1 = *0.780 ± 0.014*）皆居全模型之冠且波動最小，展現**極佳的穩定度與分類力**；而 **CatBoost**（CV AUC = *0.761 ± 0.022*）則具備**最強的風險機率排序能力**。相較之下，**隨機森林**與 **ExtraTrees** 等傳統樹模型因預設深度易對訓練集過擬合（Acc 降至 *0.666–0.681*），表現不及具正規化機制的 **Boosting** 模型。綜合來看，追求高準確率與臨床高可解釋性首選 **羅吉斯迴歸**，若著重風險排序則以 **CatBoost** 占優。
